...
for it in range(0, 400):
    ...
    itFlashes = np.reshape([0]*12*12, (12, 12))

    while keepGoing == True and reallyStop == False:
        flashes = matrix >= 10
        thisFlashes = sum(sum(flashes))
        itFlashes = itFlashes + flashes
        if sum(sum(itFlashes)) == 100:
            print("Fisrt synchronizing at step", it + 1)
            reallyStop = True
